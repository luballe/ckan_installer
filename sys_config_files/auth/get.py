...
@logic.validate(logic.schema.default_package_list_schema)
def ider_current_package_list_with_resources(context, data_dict):
    '''Return a list of the site's datasets (packages) and their resources.
    The list is sorted most-recently-modified first.
    :param limit: if given, the list of datasets will be broken into pages of
        at most ``limit`` datasets per page and only one page will be returned
    ...
    for p in packages:
        id_metadato = p['name']
        resumen = p['notes']
        entidad = p['organization']['title']
        groups = []
        groups = p['groups']
        log.warning('groups')
        log.warning(groups)
        log.warning(len(groups))
        categoria=''
        id_categoria=''
        if len(groups) > 0 :
            categoria = groups[0]['display_name']
            id_categoria =  groups[0]['id']
        tags = []
        tags = p['tags']
        keywords = ''
        for t in tags:
        ...
        if len(keywords) > 2:
            keywords = keywords[:-2]
        es_servicio_seguro = False
        es_servicio_publico = False
        es_mapa_base = False
        es_servicio_buscador = False
        extras = []
        extras = p['extras']
        for e in extras:
            if (e['key'] == "Mapa Base" or e['key'] == "mapa base") and e['value'] == '1':
                es_mapa_base = True
            elif (e['key'] == "Servicio Buscador" or e['key'] == "servicio buscador") and e['value'] == '1':
                es_servicio_buscador = True
            elif (e['key'] == "Servicio Publico" or e['key'] == "servicio publico") and e['value'] == '1':
                es_servicio_publico = True
            elif (e['key'] == "Servicio Seguro" or e['key'] == "servicio seguro") and e['value'] == '1':
                es_servicio_seguro = True
        resources = []
        ...
        for r in resources:
            ...
            fecha = r['created']
            tipo = ''
            if r['format'] == 'Esri REST' or r['format'] == 'esri rest':
                tipo = 'FeatureLayer'
            if r['format'] == 'WMS' or r['format'] =='wms':
                tipo = 'WMSLayer'
            if tipo == 'FeatureLayer' or tipo == 'WMSLayer' :
                resource_ider = {"ENTIDAD": entidad,
                       "ID_SERVICIO": id_servicio,
                       "FECHA": fecha,
                       "ES_SERVICIO_PUBLICO" : es_servicio_publico,
                       "URL" : url,
                       "CONFIG_POPUP" : " ",
                       "ID_METADATO": id_metadato,
                       "TIPO" : tipo,
                       "ES_SERVICIO_SEGURO" : es_servicio_seguro,
                       "RESUMEN" : resumen,
                       "ID_CATEGORIA" : id_categoria,
                       "ES_MAPA_BASE" : es_mapa_base,
                       "NOMBRE" : nombre,
                       "KEYWORDS" : keywords,
                       "CATEGORIA" : categoria,
                       "ES_SERVICIO_BUSCADOR" : es_servicio_buscador
                }
                #json_a.append(resource_ider)
                if categoria != "" and len(keywords)>0:
                    if es_mapa_base:
                        json_basemaps.append(resource_ider)
                    else:
                        json_services.append(resource_ider)

    #json_a.sort()
    json_services.sort()
    json_basemaps.sort()
    #json_result['current']=json_a
    json_result['services']=json_services
    json_result['basemaps']=json_basemaps
...
def user_auth_show(context, data_dict):
    '''Return a user account.
    ...
    :param id: the id or name of the user (optional)
    :type id: string
    :param passw: the password fo the user to authenticate
    :type passw: string
    :param user_obj: the user dictionary of the user (optional)
    :type user_obj: user dictionary
    :param include_datasets: Include a list of datasets the user has created.
    ...
    _check_access('user_auth_show', context, data_dict)

    id = data_dict.get('id', None)
    provided_user = data_dict.get('user_obj', None)
    passw = data_dict.get('passw', None)
    if id:
        user_obj = model.User.get(id)
        context['user_obj'] = user_obj
        if user_obj is None:
    ...
    valid = user_obj.validate_password(passw)

    if valid:
        log.warning("Success")
        log.warning(user_obj)
    else:
        log.warning("nop")
    ...
    include_private_and_draft_datasets = True

    #context['count_private_and_draft_datasets'] = False;
    #    include_private_and_draft_datasets
    ...
    if valid:
        log.warning("Success")
        log.warning(user_obj)
        user_dict['apikey'] = user_obj.apikey
